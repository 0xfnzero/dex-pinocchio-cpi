//! CPI module for Stabble CLMM
//!
//! Program: amm_v3
//! Program ID: 6dMXqGZ3ga2dikrYS9ovDXgHGh5RUsb2RTUj6hrQXhk6
//! Instructions: 21
...
/// Program ID
pub const ID: Address = Address::new_from_array(five8_const::decode_32_const("6dMXqGZ3ga2dikrYS9ovDXgHGh5RUsb2RTUj6hrQXhk6"));

// ============================================
// Instruction Discriminators
// ============================================
pub const CLOSE_POSITION: [u8; 8] = [123, 134, 81, 0, 49, 68, 98, 98];
pub const CLOSE_PROTOCOL_POSITION: [u8; 8] = [201, 117, 152, 144, 85, 85, 108, 178];
...
/// Accounts for `close_protocol_position`
pub struct CloseProtocolPositionAccounts<'a> {
    /// admin
...
impl<'a> CloseProtocolPositionAccounts<'a> {
    /// Convert to InstructionAccount array
    #[inline(always)]
    ...
    pub fn to_views(&self) -> [&'a AccountView; 2] {
        [
            self.admin, self.protocol_position
        ]
    }
}
...
impl<'a> DecreaseLiquidityV2Accounts<'a> {
    ...
    pub fn to_instruction_accounts(&self) -> [InstructionAccount<'a>; 16] {
        [
            ...
            InstructionAccount::writable(self.personal_position.address()),
            InstructionAccount::writable(self.pool_state.address()),
            InstructionAccount::readonly(self.protocol_position.address()),
            InstructionAccount::writable(self.token_vault_0.address()),
            InstructionAccount::writable(self.token_vault_1.address()),
...
impl<'a> InitializeRewardAccounts<'a> {
    ...
    pub fn to_views(&self) -> [&'a AccountView; 10] {
        [
            self.reward_funder, self.funder_token_account, self.amm_config, self.pool_state, self.operation_state, self.reward_token_mint, self.reward_token_vault, self.reward_token_program, self.system_program, self.rent
        ]
    }
}
...
/// CPI: close_protocol_position
#[inline(always)]
pub fn close_protocol_position<'a>(
    accounts: &CloseProtocolPositionAccounts<'a>,
    signers: &[Signer<'_, '_>],
) -> ProgramResult {
    // Build instruction data
    let mut data = [0u8; 8 + 0];
    data[0..8].copy_from_slice(&CLOSE_PROTOCOL_POSITION);
